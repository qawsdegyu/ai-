...
export const appRouter = router({
  ...
  auth: router({
    ...
    localLogin: publicProcedure.input(z.object({ email: z.string().email(), password: z.string() })).mutation(async ({ input, ctx }) => {
      ...
      const supabaseUrl = ENV.supabaseUrl || SUPABASE_URL;
  ...
  admin: router({
    ...
    createUser: adminProcedure.input(z.object({ name: z.string().min(1), email: z.string().email(), password: z.string().min(6), role: z.enum(["admin", "supervisor", "technician", "viewer"]) })).mutation(async ({ input }) => {
      ...
      if (!data.user) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to create confirmed user in Supabase" });
