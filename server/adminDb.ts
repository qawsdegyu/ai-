...
export function buildRoleAuditEntry(targetUserId: number, targetName: string | null, previousRole: AppRole, role: AppRole, actor: { id: number; name?: string | null }) {
...
export async function updateUserRole(targetUserId: number, role: AppRole, actor: { id: number; name?: string | null }) {
  const db = await getDb();
  ...
  await db.insert(auditLogs).values(buildRoleAuditEntry(targetUserId, target.name, target.role as AppRole, role, actor));
  return { ...target, role };
}
