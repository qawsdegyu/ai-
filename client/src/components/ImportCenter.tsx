  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6 items-start max-w-6xl mx-auto">
      ...
      <Card className="border-0 shadow-[0_16px_40px_rgba(20,128,74,0.07)] relative overflow-hidden">
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-xl text-[#e2e8f0]">
            <Database className="text-[#22c7a7]" /> Router Records
          </CardTitle>
        ...
        <CardContent className="space-y-5 relative pt-2">
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-[#22c7a7]">1. Select Data Type</Label>
            <Select value={sourceType} onValueChange={setSourceType}>
              <SelectTrigger className="border-[#dbece2] bg-[#091a2d] shadow-inner text-sm h-11">
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-[#22c7a7]">2. Select File</Label>
            <div className={`relative flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-8 transition-colors ${file ? "border-[#22c7a7] bg-[#f3fbf6]" : "border-[#bde4ca] bg-[#091a2d] hover:bg-[#10243a]"}`}>
              ...
              <div className="flex flex-col items-center text-center text-sm">
                <UploadCloud size={32} className={`mb-3 ${file ? "text-[#22c7a7]" : "text-[#80cda3]"}`} />
                {file ? <div className="font-semibold text-[#22c7a7]">{file.name}</div> : <div className="font-medium text-slate-400">Click to browse or drag and drop<br /><span className="text-xs text-slate-400 mt-1 block">Supports .xlsx, .xls, .xlsm, .csv</span></div>}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Button type="button" variant="outline" onClick={() => setLocation("/editor")} className="w-full h-11 border-[#22c7a7] text-[#22c7a7] hover:bg-[#10243a]">
