    ...
    <Card className="border-green-100 shadow-sm">
      ...
      <CardContent className="pt-4">
        <div className="flex items-center justify-between mb-4">
          <p className="text-xs text-slate-400 font-medium bg-slate-100 px-2 py-1 rounded">
            {status.data.email}
          </p>
        ...
        {files.isLoading ? (
          ...
          <div className="border rounded-md divide-y overflow-hidden">
              ...
              <div key={item.id} className="flex flex-col border-b last:border-b-0 hover:bg-slate-50 transition-colors">
                <div className="flex items-center justify-between p-3">
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <span className="text-sm truncate font-medium text-slate-200">{item.name}</span>
                  </div>
                ...
                {item.syncStatus && item.syncStatus !== "discovered" && !item.folder && (
                  <div className="px-10 pb-3 -mt-1 flex items-center gap-4 text-[10px] text-slate-400">
                    {item.syncStatus === "active" && (
                      <>
                        <span><strong className="text-slate-200">{item.sheetCount}</strong> sheets</span>
                        <span><strong className="text-slate-200">{item.indexedCells}</strong> cells indexed</span>
            ...
            {files.data?.length === 0 && (
              <div className="p-4 text-center text-sm text-slate-400">This folder is empty.</div>
            )}
          </div>
        )}
      </CardContent>
