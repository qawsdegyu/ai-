...
import SearchPerformancePanel from "@/components/SearchPerformancePanel";

const AdminUsers = lazy(() => import("@/components/AdminUsers"));
const AiAnalytics = lazy(() => import("@/components/AiAnalytics"));
const ImportCenter = lazy(() => import("@/components/ImportCenter"));
      ...
      {location === "/" && <header className="aviation-hero relative overflow-hidden px-6 pb-7 pt-6 lg:px-10">
        ...
        <div className="relative mx-auto max-w-[1500px]">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3"><div className="logo-mark"><Plane size={22} /></div><div className="flex items-center gap-2"><div className="text-xl font-bold tracking-tight text-white">Imcan Team</div></div></div>
...
<ArchiveRestore size={14} /> : <Trash2 size={14} />}</Button><Button size="icon" variant="outline" className="h-7 w-7 bg-[#0ea5e9]/10 text-[#38bdf8] border-[#38bdf8]/30 hover:bg-[#0ea5e9]/20" onClick={() => { setConversationId(undefined); setActiveConversationId(undefined); setAiMessages([]); }} title="New Chat"><Plus size={14} /></Button></div></div></CardHeader><CardContent className="flex-1 overflow-y-auto p-4 space-y-2">{aiConversations.isLoading ? <div className="h-10 animate-pulse rounded-lg bg-[#0f172a]" /> : aiConversations.data?.length ? aiConversations.data.map(item => <div key={item.id} className={`group relative rounded-lg border p-3 transition-colors ${activeConversationId === item.id ? "border-[#38bdf8] bg-[#061124]" : "border-transparent hover:bg-[#0f172a]"}`}><button type="button" onClick={() => setActiveConversationId(item.id)} className="w-full text-left text-sm pr-12"><div className="truncate font-medium text-[#e2e8f0]">{item.title}</div><div className="mt-1 text-[11px] text-[#64748b]">{new Date(item.updatedAt).toLocaleString()}{item.archivedAt ? " · Archived" : ""}</div></button><div className={`absolute right-2 top-2 flex flex-col gap-1 ${activeConversationId === item.id ? "opacity-100" : "opacity-0 lg:group-hover:opacity-100"} transition-opacity`}><Button type="button" size="icon" variant="ghost" className="h-6 w-6 text-[#64748b] hover:text-[#38bdf8]" title={item.archivedAt ? "Restore" : "Archive"} onClick={() => setConfirmAction({ type: "archive", id: item.id, title: item.title, isArchived: !!item.archivedAt })}>{item.archivedAt ? <ArchiveRestore size={12} /> : <Archive size={12} />}</Button><Button type="button" size="icon" variant="ghost" className="h-6 w-6 text-red-500 hover:text-red-400 hover:bg-red-950" title="Delete" onClick={() => setConfirmAction({ type: "delete", id: item.id, title: item.title })}><XCircle size={12} /></Button></div></div>) : <p className="text-sm text-center mt-10 text-[#64748b]">No saved logs.</p>}</CardContent></Card><div className="min-w-0 flex-1 flex flex-col bg-[#0b1527] rounded-xl border border-[#1e293b] shadow-[0_0_30px_rgba(14,165,233,0.1)] overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#1e293b] bg-[#061124] px-5 py-3"><div className="flex items-center gap-2 text-[#38bdf8] font-semibold"><Sparkles size={18} />{activeConversationId ? aiConversations.data?.find(c => c.id === activeConversationId)?.title || "AI Autopilot" : "Engage AI Autopilot"}</div><div className="flex flex-wrap items-center gap-2 text-[10px] font-semibold uppercase tracking-wide text-slate-400"><span className="border border-[#38bdf8]/30 px-2 py-1 text-[#7de7ff]">Exact Match</span><span className="border border-[#65d892]/30 px-2 py-1 text-[#65d892]">Cache 5 min</span><span className="border border-[#f5c45e]/30 px-2 py-1 text-[#f5c45e]">5 Indexes</span></div></div>{assistantFocus && <div className="shrink-0 border-b border-[#38bdf8]/30 bg-[#071c31] px-5 py-3 text-xs text-slate-300"><div className="flex flex-wrap items-center gap-x-3 gap-y-1"><span className="font-semibold text-[#7de7ff]">Answer location</span><span>File: <strong className="text-white">{assistantFocus.file}</strong></span><span>Sheet: <strong className="text-white">{assistantFocus.sheet}</strong></span><span>Cell: <strong className="font-mono text-[#7de7ff]">{assistantFocus.cell}</strong></span><span>Row: <strong className="text-white">{assistantFocus.row || "—"}</strong></span><span>Column: <strong className="text-white">{assistantFocus.column || "—"}</strong></span></div></div>}<div className="flex-1 overflow-hidden relative"><AIChatBox messages={aiMessages} onSendMessage={handleAiSend} isLoading={aiAsk.isPending} height="100%" placeholder="Transmit query to flight computer..." emptyStateMessage="Engage the Flight Deck AI Autopilot" suggestedPrompts={["Identify Site: VAPAMM001", "Network", "Incident", "LAN", "Request", "SITATEX"]} onFeedback={(_, value) => toast.success(value === "helpful" ? "Thanks — answer marked helpful." : "Thanks — we will review this answer.")} /></div></div></div> : location === "/analytics" ? (user?.role === "admin" ? <AiAnalytics /> : <OverviewSummary rows={rows} total={stats.data?.total ?? 0} migrated={stats.data?.migrated ?? 0} notMigrated={stats.data?.notMigrated ?? 0} />) : location === "/" ? <OverviewSummary rows={rows} total={stats.data?.total ?? 0} migrated={stats.data?.migrated ?? 0} notMigrated={stats.data?.notMigrated ?? 0} /> : <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_310px]">
      ...
      <Dialog open={!!selected} onOpenChange={open => !open && setSelected(null)}><DialogContent className="w-[calc(100vw-2rem)] max-w-[980px] max-h-[90vh] overflow-x-hidden overflow-y-auto border-[#17324b] bg-[#0b1527] text-[#e2e8f0] shadow-[0_24px_80px_rgba(0, 0, 0, 0.45)]"><DialogHeader><div className="mb-2 flex items-center gap-2"><Badge className={selected?.migrationStatus === "Migrated" ? "bg-[#0c2d38] text-[#22c7a7] hover:bg-[#0c2d38]" : "bg-[#332a12] text-[#f5c45e] hover:bg-[#332a12]"}>{selected?.migrationStatus}</Badge><span className="text-xs text-slate-400">{selected?.source}</span></div><DialogTitle className="break-words pr-8 text-2xl font-bold text-[#e2e8f0]">{display(selected?.routerName)}</DialogTitle><DialogDescription className="sr-only">Router details and migration information</DialogDescription><p className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-slate-400"><MapPin size={14} className="shrink-0" /> <span className="break-words">{display(selected?.city)}, {display(selected?.country)} · <span className="font-mono text-[#7de7ff]">{display(selected?.siteId)}</span></span></p></DialogHeader>{selected && <div className="mt-3 grid gap-4 sm:grid-cols-2">{[[labels?.contactDetails ?? "Contact Details", selected.contactDetails], [labels?.location ?? "Location", selected.location], [labels?.operationalHours ?? "Operational Hours", selected.operationalHours], [labels?.proactiveEmailContacts ?? "Proactive Email Contacts", selected.proactiveEmailContacts], [labels?.switchName ?? "Switch Name", selected.switchName], [labels?.subnetIp ?? "Subnet IP", selected.subnetIp], [labels?.mcsStatus ?? "MCS Status", selected.mcsStatus], ["Circuit Type", selected.circuitType]].map(([label, value], index) => <div key={`${label}-${index}`} className={`min-w-0 rounded-xl border border-[#17324b] bg-[#091a2d] p-4 ${index === 0 || index === 3 ? "sm:col-span-2" : ""}`}><div className="mb-2 break-words text-[10px] font-bold uppercase tracking-[0.14em] text-[#22c7a7]">{label}</div><div className="break-words whitespace-pre-wrap [overflow-wrap:anywhere] text-sm leading-6 text-slate-300">{display(value)}</div></div>)}</div>}</DialogContent></Dialog>
    </div>
